...
let validString = /\s/; //validating the string for spaces
...
exports.authordata = async (req, res) => {
    try {
        ...
        if (!data.emailId || validString.test(data.emailId)) return res.status(404).send({ status: false, msg: "email missing" })
        if (!data.password || validString.test(data.password)) return res.status(404).send({ status: false, msg: "password missing" })
...
exports.loginauthor = async function (req, res) {
    try {
        ...
        if(!userName || validString.test(userName) ) return res.status(400).send({ status: false, msg: "Email is required" });
  
        if(!passWord || validString.test(passWord)) return res.status(400).send({ status: false, msg: "Password is required" });
    
         //checking if the email is valid by using email-validator package
    if(!validateEmail.validate(userName)) return res.status(400).send({ status: false, msg: "Enter a valid email" })
